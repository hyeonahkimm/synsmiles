...
class SynthesizabilityEvaluator:
    def __init__(self, num_workers: int = 4, invalid: float = 0.0, max_size: int = 50_000, use_retrosynthesis: bool = False, sa_threshold: float = 4.0, env: str = 'stock', max_steps: int = 2):
        if use_retrosynthesis:
            env_dir = Path('/network/scratch/k/kimh/projects/RxnFlow/data/envs/' + env)
    ...
    def score_batch(self, objs: list[str] | list[Chem.Mol]) -> list[float]:
        # INSERT_YOUR_CODE
        # Convert list of molecules to SMILES if necessary
        if isinstance(objs, list) and len(objs) > 0 and isinstance(objs[0], Chem.Mol):
            smiles_list = [Chem.MolToSmiles(mol) for mol in objs]
        else:
            smiles_list = objs
...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def __init__(
        ...
        self.use_reward_shaping = cfg.algo.use_reward_shaping
        self.auxiliary_loss_type = cfg.algo.auxiliary_loss_type
        self.auxiliary_loss_weight = cfg.algo.auxiliary_loss_weight
        self.filter_unsynthesizable = cfg.algo.filter_unsynthesizable
        # if cfg.algo.use_reward_shaping or self.auxiliary_loss_type != "none":
        self.synthesizability_evaluator = SynthesizabilityEvaluator(use_retrosynthesis=cfg.algo.use_retrosynthesis, sa_threshold=3.0, env=cfg.algo.retro_env, max_steps=cfg.algo.max_retro_steps)
    ...
    def construct_batch(self, trajs, cond_info, log_rewards):
        ...
        batch.is_onpolicy = torch.tensor([i.get("is_onpolicy", True) for i in trajs]).float()
        # if self.synthesizability_evaluator:
        batch.is_synthesizable = torch.tensor([i.get("is_synthesizable", True) for i in trajs]).float()
        # else:
        #     batch.is_synthesizable = torch.zeros_like(batch.is_valid)
    ...
    def compute_batch_losses(
        ...
        if self.filter_unsynthesizable and batch.is_synthesizable.sum() == 0:
            return None, {}
        ...
        if self.use_reward_shaping:
            log_rewards[batch.is_synthesizable == 0] = self.global_cfg.algo.illegal_action_logreward
        # Clip rewards
        ...
        if self.filter_unsynthesizable:
            tb_loss = traj_losses[batch.is_synthesizable == 1].mean()
        else:
        ...
        info = {
            ...
            "synthesizability": (batch.is_synthesizable * batch.is_onpolicy).mean(),
        }
        if self.ctx.has_n() and self.cfg.do_predict_n:
    ...
    def compute_batch_aux_losses(
        ...
        if batch.is_synthesizable.mean() < 0.05 or batch.is_synthesizable.mean() > 0.95:
            return None, {"pos_logp": 0.0, "neg_logp": 0.0, "aux_loss": 0.0}
        ...
        neg_log_sum = torch.logsumexp(traj_log_p_F[batch.is_synthesizable == 0], dim=0)
        pos_logp = traj_log_p_F[batch.is_synthesizable == 1]
        log_denominator = torch.logaddexp(pos_logp, neg_log_sum) - math.log(max(neg_log_sum.numel(), 1.0))
        aux_loss = -(pos_logp - log_denominator)
        info = {"pos_logp": traj_log_p_F[batch.is_synthesizable == 1].mean().item(), 
                "neg_logp": traj_log_p_F[batch.is_synthesizable == 0].mean().item(), 
                "aux_loss": aux_loss.mean().item()}
        return aux_loss.mean(), info
