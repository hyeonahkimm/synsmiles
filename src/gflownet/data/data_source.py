...
class DataSource(IterableDataset):
    ...
    def do_sample_model(self, model, num_from_policy, num_new_replay_samples=None):
        ...
        def iterator():
            while self.active:
                ...
                self.compute_properties(trajs, mark_as_online=True, mark_as_onpolicy=True)
    ...
    def compute_properties(self, trajs, mark_as_online=False, mark_as_onpolicy=False):
        ...
        # if self.algo.synthesizability_evaluator:
        synth = self.algo.synthesizability_evaluator.score_batch(objs)
        ...
        for i in range(len(trajs)):
            ...
            trajs[i]["is_synthesizable"] = synth[i]
            trajs[i]["is_onpolicy"] = mark_as_onpolicy
            ...
            if i not in valid_idcs:
                ...
                trajs[i]["is_synthesizable"] = False
    ...
    def send_to_replay(self, trajs):
        if self.replay_buffer is not None:
            for t in trajs:
                if (self.algo.filter_unsynthesizable or self.cfg.algo.use_reward_shaping) and not t["is_synthesizable"]:
                    continue
                t_ = t.copy() 
                t_["is_onpolicy"] = False
                self.replay_buffer.push(t_, t_["log_reward"], t_["obj_props"], t_["cond_info"], t_["is_valid"])
