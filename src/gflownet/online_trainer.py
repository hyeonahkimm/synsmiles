...
class StandardOnlineTrainer(GFNTrainer):
    ...
    def setup(self):
        ...
        self.sampling_hooks.append(AvgObjHook())
        self.valid_sampling_hooks.append(AvgRewardHook())
        self.valid_sampling_hooks.append(AvgObjHook())
    ...
    def step(self, loss: Tensor, aux_loss: Tensor | None = None):
        loss.backward()
        with torch.no_grad():
        ...
        if aux_loss is not None:  # Hyeonah: separated update for auxiliary loss
            aux_loss.backward()
            self.opt.step()
            self.opt.zero_grad()
            self.opt_Z.step()
...
class AvgObjHook:
    def __call__(self, trajs, rewards, obj_props, extra_info):
        if obj_props.size(1) == 1:
            flat_obj_props = obj_props.squeeze(1)
        else:
            flat_obj_props = obj_props.mean(dim=1)
        sorted_obj_props, _ = torch.sort(flat_obj_props, descending=True)
        top1_avg = sorted_obj_props[:1].mean().item() if len(sorted_obj_props) >= 1 else 0.0
        top10_avg = sorted_obj_props[:10].mean().item() if len(sorted_obj_props) >= 10 else 0.0
        top100_avg = sorted_obj_props[:100].mean().item() if len(sorted_obj_props) >= 100 else 0.0
        
        return {"sampled_obj_avg": flat_obj_props.mean().item(), 
                "sampled_obj_top1_avg": top1_avg,
                "sampled_obj_top10_avg": top10_avg,
                "sampled_obj_top100_avg": top100_avg}