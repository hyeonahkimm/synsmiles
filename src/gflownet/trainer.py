...
class GFNTrainer:
    ...
    def train_batch(self, batch: gd.Batch, epoch_idx: int, batch_idx: int, train_it: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # else:
            #     step_info = None
            if self.cfg.algo.use_auxiliary_loss:  # Hyeonah
                aux_loss, aux_info = self.algo.compute_batch_aux_losses(self.model, batch)
                info.update(aux_info)
                if aux_loss:
                    loss = loss + self.cfg.algo.auxiliary_loss_weight * aux_loss
                    # _ = self.step(self.cfg.algo.auxiliary_loss_weight * aux_loss)
                    # self.algo.step()
            if loss:
                if not torch.isfinite(loss):
                    raise ValueError("loss is not finite")
