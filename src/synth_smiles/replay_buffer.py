...
class ReplayBuffer:
    ...
    def sample(self, n: int, device: str, reward_prioritized: bool = False, return_synth: bool = False, replace: bool = True):
        ...
        if reward_prioritized:
            ...
            # Avoid negative or zero rewards for sampling
            min_reward = rewards.min().item()
            if min_reward <= 0:
                rewards = rewards - min_reward + 1e-6
            probs = rewards / rewards.sum()
            indices = torch.multinomial(probs, n, replacement=replace).tolist()
        ...
        if return_synth:
            ...
            return {"input_ids": ids, "mutation_mask": mask}, rewards, synthesizabilities
        return {"input_ids": ids, "mutation_mask": mask}, rewards
