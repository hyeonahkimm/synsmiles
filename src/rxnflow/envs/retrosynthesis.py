...
class MultiRetroSyntheticAnalyzer:
    def __init__(self, analyzer, num_workers: int = 4):
        self.pool = mp.Pool(processes=num_workers, initializer=MultiRetroSyntheticAnalyzer._init_worker, initargs=(analyzer,))
        self.futures = []
    ...
    def result(self) -> list[tuple[int, RetroSynthesisTree]]:
        try:
            result = [future.get() for future in self.futures]
            self.futures = []
            return result
